...
import torch
import random 
from .target_dataset import TargetClassAwareDataset,Target_all_unlabel_ClassAwareDataset,Target_reliable_ClassAwareDataset
from .randaugment import TransformFixMatch,RandAugmentMC
...
def get_aug_transform(train=True):
    if train:
        transforms = Compose([
            Resize(256),
            RandomCrop(224),
...
def get_dataset(config, dataset, class_set, label_list=None, test=False, batch_size=None, plabel_dict=None, get_loader=True, length=None, binary_label=None, class_wise=False, validate=False):
    ...
    print(root_path)
    if class_wise:
        TargetClassAwareDataset(root_path, config.num_pclass, transforms, class_set, plabel_dict, num_steps=length*config.num_sample)
    else:
...
def init_pair_dataset(config, src_wei=None, label_list=None, plabel_dict=None, length=None, binary_label=None):
    ...
    print("src_loader size",len(src_loader.dataset))
    ...
    print("tgt_loader size",len(tgt_loader.dataset))
...
def init_reliable_target_dataset(config, plabel_dict, tgt_class_set, length=None, uk_list=None, binary_label=None):
    transform = get_transform()
    if config.task =='visda' or config.task=='imagenet-caltech':
        root_path = osp.join(config.root[config.task], config.source)
    else:
    ...
    if uk_list is not None:
        tgt_class_set.append(config.num_classes)
    dataset = Target_reliable_ClassAwareDataset(root_path, config.num_pclass, transform, tgt_class_set, plabel_dict, num_steps=length*config.num_sample)
    dataloader = DataLoader(dataset=dataset, batch_size=config.num_sample, num_workers=config.num_workers, shuffle=True, drop_last=True)
    dataloader = enumerate(dataloader)
    return dataloader

def init_unlabel_target_dataset(config, plabel_dict, tgt_class_set, length=None, uk_list=None, binary_label=None):
    if config.task =='visda' or config.task=='imagenet-caltech':
        root_path = osp.join(config.root[config.task], config.source)
    else:
        root_path = config.root[config.task]
    if uk_list is not None:
        tgt_class_set.append(config.num_classes)
    dataset = Target_all_unlabel_ClassAwareDataset(root_path, config.num_pclass, TransformFixMatch(), tgt_class_set, plabel_dict, num_steps=length*config.num_sample)
    dataloader = DataLoader(dataset=dataset, batch_size=config.num_sample, num_workers=config.num_workers, shuffle=True, drop_last=True)
    dataloader = enumerate(dataloader)
    return dataloader