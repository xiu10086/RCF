...
def create_image_label(image_list,class_set=None):
    if class_set == None:
...
class Imagelists(torch.utils.data.Dataset):
    def __init__(
        ...
        # print(image_list)
        imgs, labels = create_image_label(image_list,class_set=class_set)
        self.imgs = imgs
        self.labels = labels
    ...
    def __getitem__(self, index):
        ...
        """
        if index >= self.len_imgs :
            index = index % self.len_imgs 
        if self.keep_in_mem:
...
datasets_path = {
    "office": "/data/hy/zx/DA_data/Office",
    "officehome": "/data/hy/zx/DA_data/OfficeHome",
    "visda": "/data/hy/zx/DA_data/VisDA",
...
def create_dataset(
    ...
    if name == 'visda':
        dataset_visda_path = os.path.join( datasets_path[name], domain)
        return Imagelists(
            f"dataset/list/{name}/{txt}.txt",
            dataset_visda_path,
...
def worker_init_seed(worker_id):
    ...
    #print('dataloader seed:',worker_id+12)
